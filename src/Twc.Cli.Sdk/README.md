...
- Plugin packages are **self-contained**: they register their own dependencies and commands
...
- `RegisterServices(services)` is called by the host first.
- `RegisterCommands(commands)` is called by the host after services so commands can use DI.
...
1. Validate compatibility (`HostCompatibility.IsCompatible(hostVersion, requiredRange)`)
...
The SDK does not define Spectre types; instead, the host adapts `ICommandRegistry` to Spectre.Console.Cli internally.

## Service registration abstraction

Plugins register services via `IServiceRegistry` (a minimal abstraction). The host can adapt it to Microsoft DI (or any
container).
